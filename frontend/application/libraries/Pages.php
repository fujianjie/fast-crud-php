...
/* 
 * 分页类
 * Pages extends  CI_Pagination Class
 * 
 * 基于CI_Pagination 修改
...
class Pages {
	...
	public function create_links()
	{
		...
		$pageNum = ceil($this->total_rows/$this->per_page);
		if($this->show_total){
			$output .= "<div class=\"clearfix mobile-only\"></div><li class=\"pageTotal\">共".$pageNum."页</li>" ;
		}
		
		if($this->jump_to){
			$output .= "<div class=\"clearfix mobile-only\"></div><li class=\"pageJump\" id=\"pageJump\">到<input type=\"text\" value=\"\" class=\"form-text form-control\"/>页 <button type=\"button\" baseUrl = \"{$base_url}\" perpage=\"{$this->per_page}\" class=\"btn btn-info\"  total = \"{$pageNum}\">跳转</button></li>" ;
			$output .='<script>$(document).ready(function(){$("body").pageJump();});</script>';
		}
