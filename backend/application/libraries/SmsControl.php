...
class SmsControl {
    ...
    function __construct() {
        ...
        $this->CI->config->load('newsms', TRUE);
        $newsms = $this->CI->config->item('newsms');
        $this->Batch = date("YmdHis");
        $this->ServerIP = $newsms['serverIP'];
        $this->ServerPort = $newsms['serverPort'];
        $this->SoftVersion = '2013-12-26';
        $this->template =  $newsms['template'];
        $this->templateId =  $newsms['template']['reg'];
        $this->Handle = fopen($this->Filename, 'a');
        $this->setAccount($newsms['account'], $newsms['accounttoken']);
        $this->setAppId($newsms['appid']);
    }
     ...
     */
    public function saveLog($mobile,$data='',$templateId='',$content=''){
        $statusCode = '';
        if(!empty($content)){
            ...
            $this->CI->db->insert('we_sms',$saveArray);
    }
...
//        }
    ...
    function sendTemplateSMS($to, $datas, $tempId= '') {
        ...
        if(empty($tempId)){
            ...
            if(isset($this->template[$tempId])){
                $tempId = $this->template[$tempId] ; 
            }
        }
        $sendStr = implode(',',$datas);
        $auth = $this->accAuth();
        if ($auth != "") {
            return $auth;
        }
...
//        }
        ...
        $this->saveLog($to, $sendStr, $tempId, $result);
        return $datas;
    }
